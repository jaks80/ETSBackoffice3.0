    ...
    public PNR airToPNR() throws ParseException {
        ...
        for (String s : air.getLines()) {
            if (s.startsWith("D-")) {
                ...
                pnr.setPnrCreationDate(DateConverter.yyMMddToDate(vals[0]));
                pnr.setAirCreationDate(DateConverter.yyMMddToDate(vals[2]));
                break;
            }
        }
    ...
    public List<Ticket> airToTicket() throws ParseException {
        ...
        for (String s : air.getLines()) {
            ...
            if (s.startsWith("K-") && s.length() > 4) {
                ...
                Date tkOKDate = DateConverter.ddmmToDate(data[0].substring(2));
                if (ticket.getTktStatus() == 2 || ticket.getTktStatus() == 3) {
                    if ("INV".equals(air.getType())) {
                        ticket.setDocIssuedate(tkOKDate);
                    } else {
                        ticket.setDocIssuedate(DateConverter.yyMMddToDate(air.getCreationDate()));
                    }
                }
            }
        }
    ...
    public List<Ticket> airToRefundedTicket() throws ParseException {
        ...
        for (String s : air.getLines()) {
            if (s.startsWith("RFD")) {
                ...
                ticket.setDocIssuedate(DateConverter.refundDate(data[1]));
            }
        }
